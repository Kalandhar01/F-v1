...
const Masonry: React.FC<MasonryProps> = ({
  ...
  const { grid, maxHeight } = useMemo<{ grid: GridItem[], maxHeight: number }>(() => {
    ...
    return { grid: newGrid, maxHeight: Math.max(...colHeights) }
  }, [columns, items, width])
    ...
    <div ref={containerRef} className="relative w-full" style={{ minHeight: maxHeight }}>
      {grid.map(item => (
        <div
          key={item.id}
          data-key={item.id}
          className="absolute overflow-hidden rounded-[10px] group"
          style={{ willChange: 'transform, width, height, opacity' }}
          onMouseEnter={() => handleMouseEnter(item.id)}
          onMouseLeave={() => handleMouseLeave(item.id)}
        >
          <div
            className="relative w-full h-full bg-cover bg-center shadow-[0px_10px_50px_-10px_rgba(0,0,0,0.2)]"
          ...
          >
            ...
            {colorShiftOnHover && (
              <div className="absolute inset-0 bg-gradient-to-tr from-pink-500/50 to-sky-500/50 opacity-0 pointer-events-none color-overlay" />
            )}
          </div>
        </div>
      ))}
