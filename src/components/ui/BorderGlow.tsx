...
function buildBoxShadow(glowColor: string, intensity: number): string {
  ...
  const layers: [number, number, number, number, number, boolean][] = [
    [0, 0, 0, 1, 100, true], [0, 0, 3, 0, 50, true],
    [0, 0, 15, 0, 30, true], [0, 0, 50, 2, 10, true],
    [0, 0, 3, 0, 50, false], [0, 0, 15, 0, 30, false],
    [0, 0, 50, 2, 10, false],
  ];
...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  useEffect(() => {
    ...
    animateValue({ ease: easeInCubic, delay: 4500, duration: 2500, start: 100, end: 60,
      onUpdate: v => setEdgeProximity(v / 100),
    });
  }, [alwaysGlow])

  const colorSensitivity = edgeSensitivity + 20;
  const isVisible = isHovered || sweepActive || alwaysGlow;
