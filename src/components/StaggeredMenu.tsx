  ...
  return (
    <div className={`sm-scope fixed top-0 left-0 w-screen h-screen z-[201] ${open ? 'overflow-hidden' : 'pointer-events-none'}`}>
      <div
        ...
        style={
          accentColor
            ? ({ '--sm-accent': accentColor, '--sm-num-opacity': 0 } as React.CSSProperties)
            : ({ '--sm-num-opacity': 0 } as React.CSSProperties)
        }
      ...
      >
        ...
        <header
          className="staggered-menu-header absolute top-0 left-0 w-full flex items-center justify-between px-[1.5em] py-3 bg-transparent pointer-events-none z-20"
        ...
        >
          ...
          >
            <span
              ref={textInnerRef}
              className="sm-toggle-textWrap relative inline-block h-0 overflow-hidden hidden"
              aria-hidden="true"
            >
            ...
            >
              <span
                ref={plusHRef}
                className="sm-icon-line absolute left-0 top-0 w-full h-[2px] bg-current rounded-[2px] will-change-transform origin-center"
              />
              <span
                className="sm-icon-line absolute left-0 top-1/2 w-full h-[2px] bg-current rounded-[2px] -translate-y-1/2 will-change-transform origin-center"
              />
              <span
                ref={plusVRef}
                className="sm-icon-line sm-icon-line-v absolute left-0 bottom-0 w-full h-[2px] bg-current rounded-[2px] will-change-transform origin-center"
              />
            </span>
          </button>
        </header>
                  ...
                  {socialItems.map((s, i) => (
                    <li key={s.label + i} className="sm-socials-item">
                      <a
                        ...
                        className="sm-socials-link text-lg font-medium no-underline relative inline-block py-[2px] transition-[color,opacity] duration-300 ease-linear"
                      >
                        {s.label}
                      </a>
                    </li>
...
.sm-scope .sm-toggle { position: relative; display: inline-flex; align-items: center; gap: 0.3rem; background: transparent; border: none; cursor: pointer; color: #fff; font-weight: 500; line-height: 1; overflow: visible; }
